...
from app.schemas.workspace import WorkspaceCreate, WorkspaceUpdate, WorkspaceStats, ProjectSummary, PromptSummary, PluginStats, CredentialStats, CredentialSummary
...
class WorkspaceService:
    ...
    def get_workspace_stats(self, workspace_id: UUID) -> Optional[WorkspaceStats]:
        ...
        recent_projects_query = self.db.query(Project).filter(
            Project.workspace_id == workspace_id
        ...
        recent_prompts_query = self.db.query(Prompt).filter(
            Prompt.workspace_id == workspace_id
        ).order_by(desc(Prompt.updated_at)).limit(5)
        recent_prompts = [
            PromptSummary(
                id=p.id,
                name=p.name,
        ...
        disabled_states = [PluginState.STOPPED, PluginState.ERROR, PluginState.REGISTERED, PluginState.INITIALIZING]
        
        plugin_stats_query = self.db.query(
            func.count(case((Plugin.state.in_(enabled_states), 1))).label('enabled'),
            func.count(case((Plugin.state.in_(disabled_states), 1))).label('disabled')
        ).filter(Plugin.workspace_id == workspace_id).first()
        ...
        total_credentials = self.db.query(func.count(Credential.id)).filter(
            Credential.workspace_id == workspace_id
        ).scalar()
        
        # Get 5 most recently updated credentials
        recent_credentials_query = self.db.query(Credential).filter(
            Credential.workspace_id == workspace_id
        ).order_by(desc(Credential.updated_at)).limit(5)
        recent_credentials = [
            CredentialSummary(
                id=c.id,
                name=c.name,
