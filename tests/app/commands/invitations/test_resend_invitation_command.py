...
def test_resend_invitation_command_success(db, setup_invitation):
    ...
    assert new_invitation.expires_at > datetime.now(timezone.utc)
...
def test_resend_invitation_command_with_expired_invitation(db, setup_expired_invitation):
...
def test_resend_invitation_command_preserves_all_data(db, setup_workspace, setup_user, faker):
    ...
    assert new_invitation.expires_at > datetime.now(timezone.utc)
    time_diff = new_invitation.expires_at - datetime.now(timezone.utc)
    assert timedelta(hours=23) <= time_diff <= timedelta(hours=25)  # Allow some variance